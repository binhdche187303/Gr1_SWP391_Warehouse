...
JOIN InventoryBatches ib ON pv.variant_id = ib.variant_id
WHERE ib.status = 'In Stock'
GROUP BY pv.variant_id, p.product_name, s.size_name;



select * from Categories c join Products p on c.category_id = p. category_id where c.category_id = 1
...
--Sắp xếp lô hàng theo hsd và ngày nhập hàng
SELECT 
    pv.variant_id,
    p.product_name,
    s.size_name,
...
JOIN Sizes s ON pv.size_id = s.size_id
JOIN InventoryBatches ib ON pv.variant_id = ib.variant_id
WHERE ib.status = 'Sold Out'
ORDER BY pv.variant_id, ib.expiration_date, ib.received_date;

select * from Orders where user_id = 1

 select * from Cart where user_id = 1
 SELECT quantity FROM Cart WHERE product_id = 1 AND user_id = 1 AND size_id = 1
