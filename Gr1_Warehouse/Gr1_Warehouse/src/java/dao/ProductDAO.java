...
public class ProductDAO extends DBContext {
    ...
    public Products getProductById(int productId) {
        Products product = null;
        String sql = "SELECT p.product_id, p.product_name, p.brand_id, b.brand_name, "
                + "p.origin, p.category_id, c.category_name, p.description "
                + "FROM Products p "
    ...
    public boolean isSkuExists(String sku) {
        ...
        String cleanedSku = sku.replaceAll("\\s+", "").toLowerCase();

        String sql = "SELECT COUNT(*) FROM ProductVariants WHERE LOWER(REPLACE(sku, ' ', '')) = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, cleanedSku);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
...
// Hàm kiểm tra Product Name đã tồn tại hay chưa
    ...
    public boolean isProductNameExists(String product_name) {
        // Loại bỏ tất cả khoảng trắng và chuyển về chữ thường
        String cleanedProductName = product_name.replaceAll("\\s+", "").toLowerCase();

        String sql = "SELECT COUNT(*) FROM Products WHERE LOWER(REPLACE(product_name, ' ', '')) = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, cleanedProductName);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    // Hàm kiểm tra Product Name đã tồn tại hay chưa
    public boolean isProductNameExists(String product_name, int currentProductId) {
        // Loại bỏ tất cả khoảng trắng và chuyển về chữ thường
        String cleanedProductName = product_name.replaceAll("\\s+", "").toLowerCase();

        String sql = "SELECT COUNT(*) FROM Products WHERE LOWER(REPLACE(product_name, ' ', '')) = ? AND product_id != ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, cleanedProductName);
    ...
    public List<Products> getBestSellerProducts() {
        List<Products> productsList = new ArrayList<>();
        String query = "WITH BestSellerProducts AS ("
                + "    SELECT TOP 12 od.product_id, SUM(od.quantity) AS total_sold "
                ...
                + "    i.image_url, c.category_name, c.category_id "
                + "FROM Products p "
                + "JOIN BestSellerProducts bs ON p.product_id = bs.product_id "
                + "JOIN Categories c ON p.category_id = c.category_id "
                + "JOIN ProductMinPrice pm ON p.product_id = pm.product_id "
                + "JOIN ProductVariants pv ON p.product_id = pv.product_id AND pv.price = pm.min_price "
                + "JOIN Sizes s ON pv.size_id = s.size_id "
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("Error fetching best seller products: " + e.getMessage());
        }
    ...
    public List<Products> getTrendingProducts() {
        List<Products> productsList = new ArrayList<>();
        String query = "WITH TrendingProducts AS ("
                + "    SELECT TOP 12 od.product_id, SUM(od.quantity) AS total_sold "
                + "    FROM OrderDetails od "
                ...
                + "    i.image_url, c.category_name, c.category_id "
                + "FROM Products p "
                + "JOIN TrendingProducts tp ON p.product_id = tp.product_id "
                + "JOIN Categories c ON p.category_id = c.category_id "
                + "JOIN ProductMinPrice pm ON p.product_id = pm.product_id "
                + "JOIN ProductVariants pv ON p.product_id = pv.product_id AND pv.price = pm.min_price "
                + "JOIN Sizes s ON pv.size_id = s.size_id "
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("Error fetching trending products: " + e.getMessage());
        }
