...
public class SizeDAO extends DBContext {
    ...
    public boolean isSizeNameExists(int currentSizeId, String sizeName) throws SQLException {
        // Loại bỏ tất cả khoảng trắng và chuyển về chữ thường
        String cleanedSizeName = sizeName.replaceAll("\\s+", "").toLowerCase();

        String sql = "SELECT COUNT(*) FROM Sizes WHERE LOWER(REPLACE(size_name, ' ', '')) = ? AND size_id != ?";

        try (PreparedStatement pstm = connection.prepareStatement(sql)) {
            pstm.setString(1, cleanedSizeName);
            pstm.setInt(2, currentSizeId);
    ...
    public boolean isSizeNameExists(String sizeName) throws SQLException {
        ...
        String sql = "SELECT COUNT(*) FROM Sizes WHERE LOWER(REPLACE(size_name, ' ', '')) = ?";
