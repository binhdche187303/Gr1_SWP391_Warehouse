...
public class resetPassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!password.equals(confirmPassword)) {
            request.setAttribute("mess", "Xác nhận mật khẩu mới phải giống mật khẩu.");
        ...
        if (!password.matches(passwordPattern)) {
            request.setAttribute("mess", "Mật khẩu mới phải chứa ít nhất 1 chữ in hoa, 1 ký tự đặc biệt, 1 số và tối thiểu 8 ký tự.");
            request.setAttribute("email", email);
            request.getRequestDispatcher("/pages/reset.jsp").forward(request, response);
            return;
        }
