    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("edit".equals(action)) {
            try {
                ...
                Products currentProduct = pd.getProductById(productId);
                // Loại bỏ khoảng trắng thừa và kiểm tra trùng tên
                String cleanedProductName = productName.replaceAll("\\s+", "").trim();

                // Kiểm tra tên sản phẩm chỉ khi tên mới khác tên hiện tại
                if (pd.isProductNameExists(cleanedProductName, productId)) {
                    request.getSession().setAttribute("success", "Cập nhật thất bại do trùng tên sản phẩm");
...
// Xử lý brand_name: loại bỏ khoảng trắng thừa
                ...
                if (pd.isSkuExists(cleanedSku)) {
                    ...
                    // Xóa các thuộc tính session đã đặt trong trường hợp thất bại
                    request.getSession().removeAttribute("errorMessage");
                    request.getSession().removeAttribute("sku");
            ...
            try {
                ...
                if (success) {
                    ...
                    request.getSession().setAttribute("success", "Cập nhật thất bại");
                }
